...
//helper function to get the video upload url
const getSessionUerId = async(): Promise<string> => {
...
const revalidatePath = (paths: string[]) => {
...
export const saveVideoDetails = withErrorHandling(async (videoDetails: VideoDetails) =>{
    ...
    const userId = await getSessionUerId();
